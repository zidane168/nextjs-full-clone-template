...
import {
    getDeliveries,
    selectDeliveries,
    selectIsLoadDeliveryList,
} from "@/redux/features/delivery/reducer"
...
const Delivery = ({ params: { locale } }: { params: ILayoutParams }) => {
    ...
    const isLoadedList = useAppSelector(selectIsLoadDeliveryList)
    const pathname = usePathname()

    const listItems = useAppSelector(selectDeliveries)
    ...
    const deleteDistrictItem = (id: number) => {
        dispatchAction(dispatch, deleteDistrict({ id }), t("message.itemIsDeleteSuccessful"))
    }

    const onDelete = (id: number) => {
        showConfirmation(t("swal.confirmTitle"), t("swal.confirmDeleteText"), () =>
            deleteDistrictItem(id),
        )
    }
